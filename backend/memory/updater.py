...
def _save_memory_to_file(memory_data: dict[str, Any], user_id: str) -> bool:
...
def reload_memory_data(user_id: str) -> dict[str, Any]:
    """Reload memory data via storage provider."""
    return get_memory_storage().reload(user_id=user_id)

def delete_memory_data(user_id: str) -> bool:
    """Reload memory data via storage provider."""
    return get_memory_storage().delete(user_id=user_id)

def clear_memory_data(user_id: str) -> dict[str, Any]:
    """Clear all stored memory data and persist an empty structure."""
    cleared_memory = create_empty_memory()
    if not _save_memory_to_file(cleared_memory, user_id=user_id):
...
def create_memory_fact(
    ...
    if not _save_memory_to_file(updated_memory, user_id=user_id):
        raise OSError("Failed to save memory data after creating fact")

    return updated_memory


def delete_memory_fact(fact_id: str, user_id: str) -> dict[str, Any]:
...
def run_async_update_sync(coro: Awaitable[bool]) -> bool:
...
def fact_content_key(content: Any) -> str | None:
...
class MemoryUpdater:
    ...
    def prepare_update_prompt(
        ...
        correction_hint = self.build_correction_hint(
            correction_detected=correction_detected,
            reinforcement_detected=reinforcement_detected,
        )
    ...
    def apply_updates(
        ...
        for section in ["recentHours", "earlierContext", "longTermBackground"]:
        ...
        for fact in new_facts:
            ...
            if confidence >= config.fact_confidence_threshold:
                ...
                fact_key = fact_content_key(normalized_content)
...
def update_memory_from_conversation(
    ...
    return updater.update_memory(messages, thread_id, correction_detected, reinforcement_detected, user_id)
