...
class TestCheckMiddleware(AgentMiddleware[CodeAgentState]):
    ...
    def test_check(self, state: CodeAgentState) -> dict | None:
        ...
        if isinstance(latest_message, AIMessage):
            if (latest_message.tool_calls and
                ...
                state["test_result"] = latest_message.tool_calls[0]['args']["content"]
                return {
                    "test_result": latest_message.tool_calls[0]['args']["content"],
                }
            elif latest_message.response_metadata["finish_reason"] and latest_message.response_metadata["finish_reason"]=="stop":
                # 这里添加 test 校验功能，更新 test_retries_time
                pass
