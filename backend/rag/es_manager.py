...
def create_es_client(address:str=get_agent_config().es_address) -> Elasticsearch:
...
def create_index(client: Elasticsearch):
    if client.indices.exists(index=get_agent_config().es_index):
        print(f"index {get_agent_config().es_index} has exists，skip creating")
        return
    ...
    index_mapping = {
        ...
        "mappings": {
            "properties": {
                ...
                "context": {"type": "text", "analyzer": "ik_cn"},
...
def build_es_bulk_actions(documents: list[tuple[str, Document]]) -> list[dict]:
    ...
    for index, document in enumerate(documents):
        ...
        action = {
            "_index": get_agent_config().es_index,
...
def es_keyword_search(client: Elasticsearch, query: str, top_k: int = 10) -> list[tuple[Document, float]]:
    """ES全文关键词检索，返回chunk_id、context、得分"""
    dsl = {
        "size": top_k,
        "query": {
            "multi_match": {
                "query": query,
                "fields": ["title^2", "context"],
                "analyzer": "ik_smart"
            }
        }
    }
    resp = client.search(index=get_agent_config().es_index, body=dsl)
    result_list : list[tuple[Document, float]] = []
    for hit in resp["hits"]["hits"]:
        src = hit["_source"]
        doc = Document(
            id=src["chunk_id"],
            page_content = src["context"]
        )
